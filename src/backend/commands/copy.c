...
{
	COPY_FILE,
	/* to/from file (or a piped program) */ // 文件或管道程序之间的复制。
	COPY_OLD_FE,
	/* to/from frontend (2.0 protocol) */ // 使用2.0协议的前端与后端之间的复制。
	COPY_NEW_FE,
	/* to/from frontend (3.0 protocol) */		  // 使用3.0协议的前端与后端之间的复制。
	COPY_CALLBACK /* to/from callback function */ // 回调函数之间的复制。
} CopyDest;

/*
 *	Represents the end-of-line terminator type of the input
 */
typedef enum EolType // 定义了输入文本的行结束类型
{
	EOL_UNKNOWN,
	EOL_NL,	 // 使用\n结束
	EOL_CR,	 // 使用\r结束
...
/*
该结构体包含执行COPY操作时使用的所有状态变量。结构体用于COPY的所有变体，
即使某些字段仅在特定情况下使用。多字节编码下，客户端编码通过设置首字节的高位来表示多字节字符，
后续字节高位可不设。扫描数据查找单字节（如ASCII）字符匹配时，需用pg_encoding_mblen()跳过多字节字符以避免误匹配。
在支持的服务器编码中，无需担心误匹配，直接比较更快。
encoding_embeds_ascii为真时表示需要采用复杂方式处理。
 */
typedef struct CopyStateData
{
	/* low-level state data */
	// 数据源/目标类型、文件句柄、消息缓冲区等低级状态数据。
	// 是否为COPY FROM、是否到达文件末尾、换行符类型等标志位
	// 字符编码相关设置，如文件编码、是否需要转码等。
	...
	// COPY命令参数，如关系表、查询描述、列号列表、文件名等。
	// 格式选项，如二进制格式、包含OID、冻结行、CSV模式及其配置项。
	...
	/*
	 * Working state for COPY TO/FROM 上下文信息
	 */
	MemoryContext copycontext; /* per-copy execution context */

	/*
	 * Working state for COPY TO 输出函数信息
	 */
	FmgrInfo *out_functions;  /* lookup info for output functions */
	MemoryContext rowcontext; /* per-row evaluation context */

	/*
	 * Working state for COPY FROM 输入函数，默认值表达式等信息
	 */
	 ...
	 * raw_buf[raw_buf_len].
	 */
#define RAW_BUF_SIZE 262144 /* we palloc RAW_BUF_SIZE+1 bytes */ // yzx修改为256KB
	char *raw_buf;
...
static CopyState BeginCopy(ParseState *pstate, bool is_from, Relation rel, // 初始化 COPY 操作，设置相关的 CopyState 状态信息。
						   RawStmt *raw_query, Oid queryRelId, List *attnamelist,
						   List *options);
static void EndCopy(CopyState cstate);										   // 结束 COPY 操作，清理和释放与 CopyState 相关的资源
static void ClosePipeToProgram(CopyState cstate);							   // 如果 COPY TO 操作是将数据传输到外部程序，这个函数用于关闭与程序的管道。
static CopyState BeginCopyTo(ParseState *pstate, Relation rel, RawStmt *query, // 用于初始化 COPY TO 操作，设置相关参数，包括文件名、是否向程序发送数据等。
							 Oid queryRelId, const char *filename, bool is_program,
							 List *attnamelist, List *options);
static void EndCopyTo(CopyState cstate);				 // 结束 COPY TO 操作，处理收尾工作并释放资源。
static uint64 DoCopyTo(CopyState cstate);				 // 执行 COPY TO 操作，将数据从数据库导出到目标文件或程序。
static uint64 CopyTo(CopyState cstate);					 // 实际的 COPY TO 操作逻辑，循环处理数据并将其输出到目标。
static void CopyOneRowTo(CopyState cstate, Oid tupleOid, // 将一行数据输出到目标位置（文件或程序）。处理数据的转换和格式化。
						 Datum *values, bool *nulls);
static void CopyFromInsertBatch(CopyState cstate, EState *estate, // 将从外部文件中读取的数据批量插入表中，适用于 COPY FROM 操作，处理批量插入和索引更新。
								CommandId mycid, int hi_options,
								ResultRelInfo *resultRelInfo, TupleTableSlot *myslot,
								BulkInsertState bistate,
								int nBufferedTuples, HeapTuple *bufferedTuples,
								uint64 firstBufferedLineNo);
static bool CopyReadLine(CopyState cstate);			   // 从输入文件或程序中读取一行文本。
static bool CopyReadLineText(CopyState cstate);		   // 以文本格式读取一行数据，处理输入的行终止符和其他字符。
static int CopyReadAttributesText(CopyState cstate);   // COPY FROM 操作中，将从一行文本中解析多个列的值。
static int CopyReadAttributesCSV(CopyState cstate);	   // 类似于 CopyReadAttributesText，但用于处理 CSV 格式的数据。
static Datum CopyReadBinaryAttribute(CopyState cstate, // 在二进制格式的 COPY FROM 操作中，读取一个二进制属性（列）的值。
									 int column_no, FmgrInfo *flinfo,
									 Oid typioparam, int32 typmod,
									 bool *isnull);
static void CopyAttributeOutText(CopyState cstate, char *string); // 将某个属性（列）的值以文本格式输出到目标文件或程序。
static void CopyAttributeOutCSV(CopyState cstate, char *string,	  // 将某个属性（列）的值以 CSV 格式输出，处理引号和其他特殊字符。
								bool use_quote, bool single_attr);
static List *CopyGetAttnums(TupleDesc tupDesc, Relation rel, // 根据列名列表和表结构获取列的索引号。
							List *attnamelist);
static char *limit_printout_length(const char *str); // 限制字符串的输出长度，通常用于错误报告或日志打印。

/* Low-level communications functions */
static void SendCopyBegin(CopyState cstate);								   // 发送 COPY 操作的开始信号给客户端或程序。
static void ReceiveCopyBegin(CopyState cstate);								   // 接收客户端发来的 COPY 开始信号，准备数据传输。
static void SendCopyEnd(CopyState cstate);									   // 发送 COPY 操作的结束信号给客户端或程序。
static void CopySendData(CopyState cstate, const void *databuf, int datasize); // 发送数据块到目标位置（文件或程序）
static void CopySendString(CopyState cstate, const char *str);				   // 发送字符串数据。
static void CopySendChar(CopyState cstate, char c);							   // 发送单个字符。
static void CopySendEndOfRow(CopyState cstate);								   // 发送一行数据的结束标志。
static int CopyGetData(CopyState cstate, void *databuf,						   // 从输入中获取数据，通常用于从文件或程序读取。
					   int minread, int maxread);
static void CopySendInt32(CopyState cstate, int32 val); // 以二进制格式发送 32 位整数。
static bool CopyGetInt32(CopyState cstate, int32 *val); // 以二进制格式接收 32 位整数。
static void CopySendInt16(CopyState cstate, int16 val); // 以二进制格式发送 16 位整数。
static bool CopyGetInt16(CopyState cstate, int16 *val); // 以二进制格式接收 16 位整数。
...
CopyFrom(CopyState cstate)
{
	...
	BulkInsertState bistate;
	uint64 processed = 0; // 记录成功处理的行数
	bool useHeapMultiInsert;
	int nBufferedTuples = 0;
	int prev_leaf_part_index = -1;

#define MAX_BUFFERED_TUPLES 10000 //yzx修改缓冲区
	HeapTuple *bufferedTuples = NULL; /* initialize to silence warning */
	Size bufferedTuplesSize = 0;	  // 批量插入缓冲区
	 ...
	 */
	/* 如果启用了 COPY FREEZE，检查当前事务状态是否允许 */
	if (cstate->freeze)
	{
		/*
 ...
 */
bool NextCopyFrom(CopyState cstate, ExprContext *econtext,
