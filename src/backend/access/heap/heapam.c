...
void heapgetpage(HeapScanDesc scan, BlockNumber page)
{
	 ...
	 */
	if (polar_bulk_read_size > 0 && ScanDirectionIsForward(scan->polar_scan_direction) && (page != scan->rs_startblock || scan->px_scan != NULL))
	{
 ...
 * ----------------
 */
void heap_rescan_set_params(HeapScanDesc scan, ScanKey key,
							bool allow_strat, bool allow_sync, bool allow_pagemode)
{
	/* adjust parameters */
	scan->rs_allow_strat = allow_strat;
 ...
 * ----------------
 */
void heap_parallelscan_initialize(ParallelHeapScanDesc target, Relation relation,
								  Snapshot snapshot)
{
					   ...
					   CommandId cid, int options, BulkInsertState bistate)
{
	...
	parallel_prepare_insert(relation, tuples, heaptuples, ntuples, xid, cid, options, 4); // 修改的代码
...
l2:
	...
	if (newbuf == buffer)
	{
		 ...
		 */
		if (hot_attrs_checked && !bms_overlap(modified_attrs, hot_attrs) && (!bms_overlap(modified_attrs, proj_idx_attrs) || ProjIndexIsUnchanged(relation, &oldtup, newtup)))
		{
			use_hot_update = true;
		}
	}
